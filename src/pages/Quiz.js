...
const Quiz = () => {
  ...
  const handleAnswer = (answer) => {
    ...
    if (currentQuestion < questions.length - 1) {
      ...
      navigate("/result", { state: { name, answers } });
    }
  };
    ...
    <div className="background">
      ...
      <div className="content">
        <div className="blur-container">
          <h2 className="quiz-title">{name}님</h2>
          <h1 className="quiz-question">{questions[currentQuestion]}</h1>
          <div className="quiz-options">
            <button className="quiz-option" onClick={() => handleAnswer("예")}>예</button>
            <button className="quiz-option" onClick={() => handleAnswer("아니오")}>아니오</button>
          </div>
          {currentQuestion > 0 && (
            <button className="prev-button" onClick={() => setCurrentQuestion(currentQuestion - 1)}>이전</button>
          )}
        </div>
      </div>
    </div>
